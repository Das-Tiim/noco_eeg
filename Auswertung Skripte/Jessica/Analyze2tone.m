function [EEGtime, dataA, dataMA,soa] = Analyze2tone(channel,fn,toneA, toneMA,ErrTr)
...
%         if n <= length(toneA)
        ...
        for zz=chanlist
            ...
            if i(n)+post <= length(EEG.data(zz,:))
                ...
                soa.A{lds+1,ch_ind+1}(size(soa.A{lds+1,ch_ind+1},1)+1,1)=str2double(EEG.event(z-1).type(4:end));
            end
            if bc=='epoc'
...
%         if n <= length(toneMA)
        ...
        for zz=chanlist
            ...
            soa.MA{1,ch_ind+1} = zz;
            if i(n)+post <= length(EEG.data(zz,:))
                dataMA{lds+1,ch_ind+1}(size(dataMA{lds+1,ch_ind+1},1)+1,:)=EEG.data(zz,i(n)-pre:i(n)+post);
                soa.MA{lds+1,ch_ind+1}(size(soa.MA{lds+1,ch_ind+1},1)+1,1)=str2double(EEG.event(z-1).type(4:end));
