...
stopLearning = 0;
filepath='/home/ngaekp/Schreibtisch/Experimente/Agency/EL2ndproject_tu_nogradual_final/EL2ndproject_tu_nogradual_final/RT/';
rtname=mainvar.sNum;
data=load([filepath '/' rtname 'RT']);
RTprev=data.allRT;
%% Sound stuff
% duration
stim.soundDur = .1;
...
%         
        ...
        % keypress)
        errorType = 0; actionPerf = 0; tempo.soundOnsetMachine=NaN; errorType2 = NaN;
...
%                 end
       ...
       %%
       mainvar.data.waitRT(tt,:)=waitingend;
       mainvar.data.RTprev(tt,:)=RTprev(tt);
        %%
        mainvar.tt=tt;
...
DrawFormattedText(display.windowPtr, ['Super! Sie haben das Training geschafft!' '\n', 'Druecken Sie die Leertaste um das Training zu beenden'], 'center', 'center', display.fgColor, [], [], [], 3);
Screen('Flip', display.windowPtr);
KbWait;
while KbCheck; end
% close all buffers
