...
if exist(mainvar.fdata,'file') %does this specific file exist? 'file': checks for files or folders
    resp = questdlg({['the file ' mainvar.fdata ' already exists']; 'do you want to overwrite or continue?'}, 'duplicate warning','cancel','overwrite','continue','continue');
    ...
    elseif strcmp(resp,'overwrite')
        mainvar.new_participant = 1;
    elseif strcmp(resp,'continue')
        load(mainvar.fdata);
