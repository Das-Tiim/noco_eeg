function [lrn] = dapa_learn2(display, mainvar, stim, keys, tempo, lrn)
...
%rand('state', sum(100*clock)); %to avoid having always the same random number per session; uniform distribution
    ...
    % catch trial list
    lrn.catchTrList{mainvar.bb} = zeros(1, mainvar.trLearn);
    randomIdx = randperm(numel(lrn.catchTrList{mainvar.bb}));
    lrn.catchTrList{mainvar.bb}(randomIdx(1:round(propCatch(randi(numel(propCatch)))*numel(randomIdx)))) = 1;
    if mainvar.bb == 1 && mainvar.training == 0
        while sum(lrn.catchTrList{mainvar.bb}(1:10)) >= 1
            lrn.catchTrList{mainvar.bb} = zeros(1, mainvar.trLearn);
            randomIdx = randperm(numel(lrn.catchTrList{mainvar.bb}));
            lrn.catchTrList{mainvar.bb}(randomIdx(1:round(.1*numel(randomIdx)))) = 1;
        end
    elseif  mainvar.bb > 1 || mainvar.training == 1
        while sum(lrn.catchTrList{mainvar.bb}(1:4)) >= 1
            lrn.catchTrList{mainvar.bb} = zeros(1, mainvar.trLearn);
            randomIdx = randperm(numel(lrn.catchTrList{mainvar.bb}));
            lrn.catchTrList{mainvar.bb}(randomIdx(1:round(propCatch(randi(numel(propCatch)))*numel(randomIdx)))) = 1;
        end
    end
    lrn.actionList{mainvar.bb} = zeros(2,  mainvar.trLearn);
...
%         PsychPortAudio('FillBuffer', low, stim.lTone);
        ...
        while isnan(wKey)
            ...
            if keyIsDown
                if (keyCode(keys.lKey) || keyCode(keys.rKey))
                    ...
                    lrn.actionList{mainvar.bb}(wKey, tt) = 1;
                    
                    % select sound to play
                    effect2play = effetto(wKey);
                    if lrn.catchTrList{mainvar.bb}(tt) == 1
        ...
        if doubleKey
            ...
            lrn.catchTrList{mainvar.bb}(end+1) = lrn.catchTrList{mainvar.bb}(tt);
            lrn.actionList{mainvar.bb}(:, end+1) = [0;0];
        end
        ...
        if mod(tt, fbActions) == 0
            resp = gef_propAction(display, lrn.actionList{mainvar.bb});
        end
        
        %% Save data
        
        lrn.pressTime{mainvar.bb}(1, tt) = pressTime; % store onsetime key press
        lrn.wKey{mainvar.bb}(1, tt) = wKey; % store key pressed
        lrn.doubleKey{mainvar.bb}(1, tt) = doubleKey;
        lrn.sound{mainvar.bb}(1, tt) = effectID(wKey);
        lrn.catchTr{mainvar.bb}(1, tt) = lrn.catchTrList{mainvar.bb}(tt);
        ...
        lrn.timingITI{mainvar.bb}(1, tt)=ITIend;
        % record data if not training session
        
        %commented the saving part
        %         if ~mainvar.training
        %             save(mainvar.fdataTr, 'lrn', 'resp');
        %         end
        startITI=tic;          
        tt=tt+1; % next trial!
        WaitSecs(tempo.blank);
    ...
    %     Fel and Elli changed on 21.10.2019
    if ~mainvar.training
        save(mainvar.fdataTr, 'lrn', 'resp');
    end
    ...
    lrn.ansCatch{mainvar.bb} = Ask(display.windowPtr, ' ? ', display.colorText,  display.bkColor, 'GetChar', [display.center(1)-replySize, display.center(2)-replySize, display.center(1)+replySize, display.center(2)+replySize], 'center', 20); 
    Screen('Flip',display.windowPtr);
    
    iscorrect = str2double(lrn.ansCatch{mainvar.bb}) == sum(lrn.catchTr{mainvar.bb});
    text4catch = {'Oops! Incorrect', 'Yes! Correct'};
    gef_phase(display, [text4catch{iscorrect+1}, ' : '  num2str(sum(lrn.catchTr{mainvar.bb}))]);
